...
import type { Day, TimelineItem } from "@/lib/engine/types";
import { DEFAULT_BEAT_SPACE } from "@/lib/engine/pacing";
import { cn } from "@/lib/engine/utils";
import { registry } from "@/trip";
    ...
    <ol className="relative">
      {day.timeline.map((item) => (
        <BeatRow
          key={item.id}
          beatId={item.id}
          span={registry.beat(day.id, item.id)?.space}
      ...
      {hideLodging ? null : (
        <BeatRow
          ...
          span={registry.beat(day.id, day.lodging.slug)?.space}
